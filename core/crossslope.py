...
class crossslope():
    def __init__(self, temp=True, parent=None):
        ...
        if temp:
            self.istemp=True
            self.object=win32com.client.Dispatch("incadopera.superelevation")
    ...
    def ptslope(self, pos):
        return self.object.PTSuperElevation(pos)
    def setptslope(self, pos, slp):
        self.object.SetPTSuperElevation(pos, slp)
    def pttype(self,pos):
        #取得横坡变化类型
    ...
    def item(self,pos):
        ...
        if pos < self.count():
            ...
            return None
    def setitem(self, pos, ch, slp):
        #TODO ,测试结果错误，可能需要修改C源代码
        if pos <self.count():
            self.object.SetItem(pos, (ch,slp))
        else:
            print "setitem error:pos超出索引范围"
    def setch(self, pos, ch):
        self.object.Setch(pos, ch)
    def ch(self, pos):
        return self.object.ch(pos)
    def slope(self, ch):
        return self.object.SuperElevation(ch)
    def getallitem(self):
