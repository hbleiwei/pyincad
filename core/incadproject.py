...
class brokench():
    ...
    def __init__(self):
        self.brdata=[]
    def append(self,prech,newch):
        #添加断链，prech为前一桩号（前一断链区间内的桩号），newch（断链后的桩号），返回断号
        self.brdata.append((prech,newch))
        return len(self.brdata)
    def calch(self,segnum,segch):
        #输入断号和断链内的桩号，计算并返回连续的桩号,第一个断号从0开始，及断链0段内为连续区间
        if (segnum > len(self.brdata) or segnum < 0):
            print "error"
            return -9999
        if (segnum == 0):
            return segch
        ...
        for i in range(0,segnum):
            dch=self.brdata[i][1]-self.brdata[i][0]+dch
        return segch-dch
    def calsegch(self,segnum,ch):
        #输入断号和连续桩号，计算返回断链内的桩号
        dch=0
        if (segnum > len(self.brdata) or segnum < 0):
            print "error"
            return -9999
        ...
        return ch+dch
    def count(self):
        return len(self.brdata) #返回区间个数
    def calsegnum(self,ch):
        #计算连续桩号的区间号
        if (self.count() == 0):
            return 0 #没有区间号，为连续区间
        if ch <= self.brdata[0][0]:
            return 0
        for i in range(0,self.count()-1):
            cch=self.calsegch(i+1,ch)
            if cch <= self.brdata[i+1][0]:
                return i+1
        return self.count() #如果都没有，则是最后一段的
class incadproject():
    def __init__(self):
    ...
    def crossset(self, rampid):
        if not self._checkproject():
            return
        con=crossset(False)
        con.object=self.project.CrossSet(rampid)
        return con
